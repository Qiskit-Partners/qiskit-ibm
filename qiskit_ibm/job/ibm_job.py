...
class IBMJob(Job):
    ...
    Job information retrieved from the server is attached to the ``IBMJob``
    ...
    def qobj(self) -> Optional[Union[QasmQobj, PulseQobj]]:
        ...
        """
        warnings.warn("The ``IBMJob.qobj()`` method is deprecated and will "
                      "be removed in a future release. You can now pass circuits "
                      "to ``IBMBackend.run()`` and use ``IBMJob.circuits()``, "
                      "``IBMJob.backend_options()``, and ``IBMJob.header()`` to retrieve "
    ...
    def result(
        ...
        Note:
            ...
            The first call to this method in an ``IBMJob`` instance will
    ...
    def cancel(self) -> bool:
        ...
        try:
            ...
            self._cancelled = False
            raise IBMJobApiError('Unexpected error when cancelling job {}: {}'
    ...
    def _get_tags_to_update(self,
        ...
        Raises:
            IBMJobInvalidStateError: If any of the input parameters are invalid.
        """
        # Tags prefix that denotes a job belongs to a jobset.
        ibm_jobset_prefix = 'ibmq_jobset_'
        ...
        if isinstance(replacement_tags, list):  # `replacement_tags` could be an empty list.
            ...
            validate_job_tags(replacement_tags, IBMJobInvalidStateError)
            tags_to_update = set(replacement_tags)
            tags_to_update.update(
                filter(lambda old_tag: old_tag.startswith(ibm_jobset_prefix), self._tags))
        if additional_tags:
            # Add the specified tags to the tags to update.
            validate_job_tags(additional_tags, IBMJobInvalidStateError)
            tags_to_update.update(additional_tags)
        if removal_tags:
            # Remove the specified tags, except those related to a job set,
            # from the tags to update.
            validate_job_tags(removal_tags, IBMJobInvalidStateError)
            for tag_to_remove in removal_tags:
                if tag_to_remove.startswith(ibm_jobset_prefix):
                    logger.warning('The tag "%s" for job %s will not be removed, because '
                                   'it is used internally by the Qiskit IBM provider.',
